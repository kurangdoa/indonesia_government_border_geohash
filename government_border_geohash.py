...
# foldername = os.path.join(path_base, 'shp','per_prov')
foldername = os.path.join(path_base, 'shp')
#listfiles = [s for s in os.listdir(foldername) if s.endswith('.kmz')]
listfiles = [s for s in os.listdir(foldername) if s.endswith('.shp')]
# listfiles = [s for s in os.listdir(foldername) if s.endswith('missing_sub_buf.shp')]
listfiles = [eachfile.split(sep = '.')[0] for eachfile in listfiles]

### start to loop per file shp ###

for eachfile in listfiles:
    # shapefile = gpd.read_file(os.path.join(path_base, 'shp', 'per_prov', eachfile+'.shp'))
    shapefile = gpd.read_file(os.path.join(path_base, 'shp', eachfile+'.shp'))
    ...
    for i in tqdm(range(len(shapefile))):
        ...
        try:
            polygon=geometry.Polygon(shapefile["geometry"][i])
        except:
            polygon=geometry.MultiPolygon(shapefile["geometry"][i])

        compiled = list(polygon_to_geohashes(polygon, 7, False)) #outer
        compiled = pd.DataFrame(list(compiled),columns=["geohash"])[0:]
        compiled["remark"] = shapefile["ADM4_PCODE"][i]
        ...
        for a,b in zip(compiled['geohash'],compiled['remark']):
            if k == 1: 
                ...
                testgeopol_compile = testgeopol
                try:
                    testgeopol_intersect = testgeopol.intersection(polygon)
                except:
                    pass
            else :
                testgeopol = geohash_to_polygon(a)
...
# plot multipolygon
# for x in geohashes_to_polygon(df['geohash']):
#     plt.plot(*x.exterior.xy)
# plt.show()

##################################
### start to loop per file kml ###
##################################
